    ...
    for file in files:
        ...
        if disc_number is not None:
            with file.open(encoding=encoding, newline='\r\n') as handle:
                disc_numbers_to_file_contents[disc_number] = handle.read()
...
def main():
    ...
    # Find LOGs
    disc_numbers_to_logs = map_disc_numbers_to_file_contents(
        music_path.glob('*.log'),
        args.log_encoding
    )

    # Find CUEs
    disc_numbers_to_cues = map_disc_numbers_to_file_contents(
        music_path.glob('*.cue'),
        args.cue_encoding
    )
